...
func (db BrandDB) GetAll() ([]models.Brand, error) {
	...
	tsql := fmt.Sprintf(query_sql.Brand["list"].Q)
...
func (db BrandDB) Get(id string) (models.Brand, error) {
	res := make([]models.Brand, 0)
	tsql := fmt.Sprintf(query_sql.Brand["get"].Q, id)
	rows, err := DB.Query(tsql)

	err = db.scan(rows, err, &res, "Brand", "GetByID")
	if err != nil {
...
func (db BrandDB) Create(item models.Brand) (int64, error) {
	...
	tsql := fmt.Sprintf(query_sql.Brand["insert"].Q)
...
func (db BrandDB) Update(id string, item models.Brand) (int64, error) {
	ctx := context.Background()
	tsql := fmt.Sprintf(query_sql.Brand["update"].Q)
	result, err := DB.ExecContext(
		ctx,
		tsql,
		sql.Named("ID", id),
...
func (db BrandDB) Delete(id string) (int64, error) {
	ctx := context.Background()
	tsql := fmt.Sprintf(query_sql.Brand["delete"].Q)
	result, err := DB.ExecContext(
		ctx,
		tsql,
		sql.Named("ID", id))
