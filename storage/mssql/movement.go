...
func (db MovementDB) GetStockLot(idWarehouse int, idProduct int, idLot int) float64 {
	var item float64
	tsql := fmt.Sprintf(query_sql.Movement["stockLot"].Q, idWarehouse, idProduct, idLot)
	rows, err := DB.Query(tsql)

	if err != nil {
		checkError(err, "GetStockLot", "Movement mssql", "Reading rows")
		return 0
	}
	for rows.Next() {
		...
		item = stock.Float64
		if err != nil {
			checkError(err, "GetStockLot", "Movement mssql", "Scan rows")
			return 0
		}
	}
	defer rows.Close()
...
func (db MovementDB) GetStockBrand(idWarehouse int, idProduct int, idBrand int) float64 {
	var item float64
	tsql := fmt.Sprintf(query_sql.Movement["stockBrand"].Q, idWarehouse, idProduct, idBrand)
	rows, err := DB.Query(tsql)

	if err != nil {
		checkError(err, "GetStockBrand", "Movement mssql", "Reading rows")
		return 0
	}
	for rows.Next() {
		var stock sql.NullFloat64
...
func (db MovementDB) GetAllLotsWarehouse(idProduct string, idWarehouse string)([]models.Movement, error) {
	...
	rows, err := DB.Query(tsql)
	if err != nil {
		checkError(err, "GetAllBrandsWarehouse", "Movement mssql", "Reading rows")
		return res, err
	}
	...
	for rows.Next() {
		...
		if err != nil {
			checkError(err, "GetAllBrandsWarehouse", "Movement mssql", "Scan rows")
		}
...
func (db MovementDB) Create(item models.Movement) (int64, error) {
	...
	product, _ := ProductDB{Ctx: "Product mssql", Query: query_sql.Product}.Get(strconv.Itoa(item.IdProduct))
...
func GetStock(idWarehouse string, idProduct int) float64 {
	...
	tsql := fmt.Sprintf(query_sql.Movement["stock"].Q, idWarehouse, idProduct)
	rows, err := DB.Query(tsql)

	if err != nil {
		checkError(err, "GetStock", "Movement mssql", "Reading rows")
		return 0
	}
	for rows.Next() {
		var stock sql.NullFloat64
		...
		item = stock.Float64
		if err != nil {
			checkError(err, "GetStock", "Movement mssql", "Scan rows")
			return 0
		}
	}
	defer rows.Close()
...
func (db MovementDB) scan(rows *sql.Rows, err error, res *[]models.Movement, ctx string, situation string) error {
	...
	for rows.Next() {
		...
		if err != nil {
			...
			product, _ := ProductDB{Ctx: "Movement", Query: query_sql.Product}.Get(strconv.Itoa(item.IdProduct))
			productMeasure, _ := ProductMeasureDB{}.GetProduct(strconv.Itoa(product.ID))
			measure, _ := MeasureDB{}.Get(strconv.Itoa(productMeasure.IdMeasure))
