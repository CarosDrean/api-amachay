...
func (db ProviderDB) GetAll() ([]models.Provider, error) {
	...
	tsql := fmt.Sprintf(query_sql.Provider["list"].Q)
...
func (db ProviderDB) Get(id string) (models.Provider, error) {
	res := make([]models.Provider, 0)
	tsql := fmt.Sprintf(query_sql.Provider["get"].Q, id)
	rows, err := DB.Query(tsql)

	err = db.scan(rows, err, &res, "provider", "GetByID")
	if err != nil {
...
func (db ProviderDB) Create(item models.Provider) (int64, error) {
	...
	tsql := fmt.Sprintf(query_sql.Provider["insert"].Q)
...
func (db ProviderDB) Update(id string, item models.Provider) (int64, error) {
	ctx := context.Background()
	tsql := fmt.Sprintf(query_sql.Provider["update"].Q)
	result, err := DB.ExecContext(
		ctx,
		tsql,
		sql.Named("ID", id),
...
func (db ProviderDB) Delete(id string) (int64, error) {
	ctx := context.Background()
	tsql := fmt.Sprintf(query_sql.Provider["delete"].Q)
	result, err := DB.ExecContext(
		ctx,
		tsql,
		sql.Named("ID", id))
